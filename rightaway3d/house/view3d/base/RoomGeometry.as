package rightaway3d.house.view3d.base
{
	...
	public class RoomGeometry extends PrimitiveBase
	{
		private var vo:Room;
		...
		public var groundGeom:CompactSubGeometry;
		public var ceilingGeom:CompactSubGeometry;
		...
		{
			_buildGeom(target,countGroundVertices,createGroundFace);
			_buildGeom(ceilingGeom,countGroundVertices,createCeilingFace);
		}
		...
		private function countGroundVertices(vs:Vector.<Vertex>,ts:Vector.<Triangle>):void
		{
			...
			//trace("countGroundVertices:"+ts);
			numIndices = ts.length * 3;
		}
		
		/*private function countCeilingVertices(vs:Vector.<Vertex>,ts:Vector.<Triangle>):void
		{
			var cws:Vector.<CrossWall> = vo.walls;
			var len:int = cws.length;
			numVertices = len;
			...
			Geom.splitPolygonByTriangle(vs.concat(),ts);
			trace("countCeilingVertices:"+ts);
			numIndices = ts.length * 3;
		}*/
		
		private function createGroundFace(data:Vector.<Number>,indices:Vector.<uint>,target:CompactSubGeometry,vs:Vector.<Vertex>,ts:Vector.<Triangle>):void
		{
			...
			for(var i:int=0;i<len;i++)
			{
				...
				u = (p.x - minX)/dx * su;
				v = (1-(p.y - minY)/dy) * sv;
				addVertexUV(data,p.x,yHeight,p.y,nx,ny,nz,tx,ty,tz,u,v);
			}
		...
		private function createCeilingFace(data:Vector.<Number>,indices:Vector.<uint>,target:CompactSubGeometry,vs:Vector.<Vertex>,ts:Vector.<Triangle>):void
		{
			var len:int = numVertices;
			var minX:Number=Infinity,minY:Number=Infinity,maxX:Number=-Infinity,maxY:Number=-Infinity;
			for(i=0;i<len;i++)
