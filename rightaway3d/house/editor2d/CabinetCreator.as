	...
	public class CabinetCreator
	{
		...
		*/		
			...
			for(var i:int=0;i<len;i++)
			{
				...
				if(info.name!=CabinetType.BAFFLE && plen>0)
				{
					...
					for(var j:int=0;j<plen;j++)
					{
						...
						var matName:String = po.customMaterialName+po.parentProductObject.memo;
		...
		private function getDoorData(pos:Array,subtotal:Object):String
		{
			...
			var productName:String = info.name;//matLib.getMaterialAttribute(name,"productModel");
			var productModel:String = matLib.getMaterialAttribute(name,"productName");
			var specifications:String = "["+(info.specifications?info.specifications:d.x+"x"+d.y+"x18")+"]x"+plen;//matLib.getMaterialAttribute(name,"specifications");
			if(po.parentProductObject.memo)specifications += "("+po.parentProductObject.memo+")";
			
			var productCode:String = matLib.getMaterialAttribute(name,"productCode");
			var c1:String = productCode.substr(0,4);
			var c2:String = info.productCode && info.productCode.length==3?info.productCode:"0XX";
...
*/		
		private function getProductsData(type:String,subtotal:Object):String
		{
			...
			for(var i:int=0;i<len;i++)
			{
				...
				//trace("getProductsData type:"+type+" num:"+plen);
				if(plen>0)
				{
		...
		public function updateCabinetTable():void
		{
			...
			createCabinetTable3(cabinetData);
...
//						var startOffset:Number = getStartOffset(cw,x0+dx0,x1+dx1,ht,et,hc,ec,gw,tableData,o)+dx0;
		...
		private function getStartOffset(cw:CrossWall,x0:Number,x1:Number,ht:String,et:String,hc:Boolean,ec:Boolean,gw:Number,tableData:WallSubArea,areaData:WallSubArea):Number
		{
			...
			if(ho && eo)//首尾皆是中高柜
			{
				if(et==ht || (ht==ObstacleType.HOLE && et==ObstacleType.NULL) || (et==ObstacleType.HOLE && ht==ObstacleType.NULL))
				{
					...
					cabinetCtr.setCabinetPos(ho.view2d,cw,x0+hw,yPos,0);//重新定位首端中高柜位置
					cabinetCtr.setCabinetPos(eo.view2d,cw,x1,yPos,0);//重新定位尾端中高柜位置
				}
			...
			else if(ho)//头部有中高柜
			{
				...
				else//对齐到首端
				{
					...
					tableData.endType = areaData.endType = ObstacleType.NULL;
					tableData.x1 = areaData.x1 = x1;
					//cabinetCtr.setCabinetPos(eo.view2d,cw,x1,0,0);//重新定位尾端中高柜位置
				}
			}
			else if(eo)//尾部有中高柜
			{
				...
				else//对齐到尾端
				{
					...
					tableData.headType = areaData.headType = ObstacleType.NULL;
					startOffset = 0;//起始位置
					//cabinetCtr.setCabinetPos(ho.view2d,cw,x0+hw,0,0);//重新定位首端中高柜位置
				}
			}
			else if(ht==et || 
			...
			/*else if(ht==ObstacleType.HOLE || ht==ObstacleType.NULL || ht==ObstacleType.WALL)//台面首端没有障碍物，或者为门洞时
			{
				if(ec)//转角柜结尾
				{
					if(cw.localEnd.x-x1<100)//转角柜至墙的间距至少为100
					{
						x1 = cw.localEnd.x - 100;
					}
				}
				if(ht==ObstacleType.WALL)
				{
					startOffset = x1 - gw - x0;
				}
				else
			...
			/*else if(et==ObstacleType.HOLE || et==ObstacleType.NULL || et==ObstacleType.WALL)//台面尾端没有障碍物，或者为门洞时
			{
				if(hc)//转角柜开头
				{
					if(x0-cw.localHead.x<100)//转角柜至墙的间距至少为100
					{
						startOffset = cw.localHead.x+100-x0;
						x0 = cw.localHead.x + 100;
					}
				}
				...
				tableData.x1 = areaData.x1 = x1;
				//trace("x0,x1,gw,startOffset:",x0,x1,gw,startOffset);
			}*/
			/*else if(hc)//转角柜开头
			{
			...
			/*else if(ht==ObstacleType.WALL)//厨柜到墙体的距离
			{
				startOffset = 40;
			}*/
			else//未处理的意外情况
		 ...
		 * 
		 */
		private function createCabinetPlate(cw:CrossWall,width:int,height:int,depth:int,xPos:Number,yPos:Number,zPos:Number,ctype:String,name:String):ProductObject
		{
			...
			else if(ctype==CabinetType.BODY_PLANK)
										   ...
										   beforeX1:Number):void
		{
			...
			for(var i:int=0;i<len;i++)
			{
				...
				this.createCabinetPlate(cw,width,80,10,start,0,520,CabinetType.LEG_BAFFLE,"柜腿挡板");//创建柜腿挡板
			}
...
//				var w:Number = x0-cw.wall.width*0.5-550+offset;//计算挡板宽度-1.5
				...
				addGroundCabinet(po);
				this.createCabinetPlate(cw,w+30,80,10,x0+offset,0,520,CabinetType.LEG_BAFFLE,"柜腿挡板");//创建拐角腿挡板，长度延伸30,至相邻插角地柜的柜腿挡板
			}
			else if(ht==ObstacleType.WALL)//柜子顶墙
			{
				...
				if(w>1)//柜子与墙之间有间隙
				{
					...
					this.createCabinetPlate(cw,w,80,10,x0+offset,0,520,CabinetType.LEG_BAFFLE,"柜腿挡板");//柜子顶墙或障碍物时，柜腿封板
				}
			}
			...
			else if(ht==ObstacleType.HOLE || ht==ObstacleType.NULL)//柜子顶门边或者什么都不靠
			{
				if(ho)
				{
					this.createCabinetPlate(cw,offset,80,10,x0+offset,0,520,CabinetType.LEG_BAFFLE,"柜腿挡板");//柜子顶墙或障碍物时，柜腿封板
					this.createCabinetPlate(cw,10,80,520,x0+10,0,0,CabinetType.LEG_BAFFLE,"柜腿封板");//柜子头上什么都没有时，侧面柜腿封板
				}
				else
				{
			...
			else if(et==ObstacleType.CORNER_CABINET || et==ObstacleType.OBJECT_CORNER_CABINET)//最尾部顶着拐角柜
			{
				...
				addGroundCabinet(po);
				this.createCabinetPlate(cw,w,80,10,tx1+w,0,520,CabinetType.LEG_BAFFLE,"柜腿挡板");//创建拐角腿挡板，长度与插角地柜等长
			}
			else if(et==ObstacleType.WALL)// || et==ObstacleType.OBJECT)
			{
				...
				if(w>1)
				{
					...
					this.createCabinetPlate(cw,w,80,10,x1,0,520,CabinetType.LEG_BAFFLE,"柜腿挡板");//柜子顶墙或障碍物时，柜腿封板
				}
			}
			...
			else if(et==ObstacleType.HOLE || et==ObstacleType.NULL)//柜子顶门边或者什么都不靠
			{
				if(eo)
				{
					w = x1 - tx1;
					this.createCabinetPlate(cw,w,80,10,x1,0,520,CabinetType.LEG_BAFFLE,"柜腿挡板");//柜子顶墙或障碍物时，柜腿封板
					this.createCabinetPlate(cw,10,80,520,x1,0,0,CabinetType.LEG_BAFFLE,"柜腿封板");//柜子头上什么都没有时，侧面柜腿封板
				}
				else
				{
		...
		private function addGroundCabinet(po:ProductObject):void
		{
			sceneGroundCabinets.push(po);
			po.addEventListener("dispose",onProductDispose);
		}
		
		private function addWallCabinet(po:ProductObject):void
		{
		...
		//计算从首端开始可以放置地柜的有效长度
		/*private function getHeadLength(cw:CrossWall):Number
		{
			var gos:Array = [];
			...
			//trace("getHeadLength:"+len);
			if(len>0)
			{
				for(var i:int=0;i<len;i++)
				{
					var wo:WallObject = gos[i];
					var n:Number = wo.x-wo.width-x0;
					//trace(i+":"+n);
					if(n>570)
					{
						return n;
		...
		//计算从尾端开始可以放置地柜的有效长度
		/*private function getEndLength(cw:CrossWall):Number
		{
			var gos:Array = [];
			var x0:Number = cw.localHead.x;
			var x1:Number = cw.localEnd.x;
			...
			var len:int = gos.length;
			//trace("getEndLength:"+len);
			if(len>0)
			{
				for(var i:int=len-1;i>-1;i--)
				{
					var wo:WallObject = gos[i];
					var n:Number = x1-wo.x;
					//trace(i+":"+n);
					if(n>570)
					{
						return n;
		...
		/*private function setGroundObjectArea2(tableData:WallSubArea,cw:CrossWall,x0:Number,x1:Number,headCorner:Boolean,endCorner:Boolean,headType:String,endType:String):void
		{
			var groundObjects:Array = [];
			tableData.groundObjects = groundObjects;
			
			var cornerWidth:int = 570+50;
			
			if(headType==ObstacleType.CORNER_CABINET)//首端相邻墙放置拐角柜
			{
				x0 += cornerWidth;//起始位置让出拐角柜的空间
			}
			
			if(endType==ObstacleType.CORNER_CABINET)//尾端相邻墙放置拐角柜
			{
				x1 -= cornerWidth;//结束位置让出拐角柜的空间
			}
			...
			//trace("----headType:"+headType,alen,tx1,tx0);
			
			if(tx1-tx0>=300+30)//判断空间是否可以放置最小规格厨柜
			{
				//trace("1");
				var o:WallSubArea = getCabinetAreaData(cw,tx0,tx1,headCorner2,endCorner2,headType2,endType2);
				groundObjects.push(o);
			}
		 ...
		 * 
		 */
		public function addSingleDoor(po:ProductObject):void
		{
		...
		private function _addSingleDoor(po:ProductObject):void
		{
			var s:String = po.productInfo.productModel.slice(0,3);
			if(s=="A30" || s=="A40" || s=="A45" || s=="B30" || s=="B40" || s=="B45" || s=="AJ9" || s=="BJ8")
			{
		...
		private function createWallCabinet(o:Object,cw:CrossWall,x:int,door:XML=null,name:String=null):Product2D
		{
			...
			var p:Product2D = cabinetCtr.createCabinet(id,file,"text",cw,x,CrossWall.WALL_OBJECT_HEIGHT,name,false,width,330);
			//if(door)
			//{
				//productManager.addDynamicSubProduct(p.vo,door);
				addSingleDoor(p.vo);
