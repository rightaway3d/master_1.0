package rightaway3d.house.utils
{
	...
	import rightaway3d.house.view2d.BaseMarking;
	...
	public class CabinetTableUtils
	{
		...
		public var view2d:Sprite;
		
		public function CabinetTableUtils()
		{
		}
		...
		//获取到房间里的所有立柱立管
		public function updateRoomPillars():void
		{
		...
		//绘制整体台面及标识水盆灶台位置
		public function getTableBorder(tables:Array,depths:Array,isView2d:Boolean):Array
		{
			...
			if(isView2d)points.push(hp);
			
			return points;
		}

		//绘制标注
		private function drawTableMark(tables:Array,depths:Array,g:Graphics,dx:int,dy:int,min:Point,max:Point):void
		{
			var drainer:ProductObject = cc.drainerProduct;
			var drainerInfo:WallObject = drainer.objectInfo;
		...
		//绘制整体台面及标识水盆灶台位置
		public function drawTable2D(tables:Array,depths:Array,g:Graphics,dx:int,dy:int,min:Point,max:Point):void
		{
			var points:Array = this.getTableBorder(tables,depths,true);
			drawPoints(g,points,dx,dy);
			
			this.drawTableMark(tables,depths,g,dx,dy,min,max);
		}
		
		//绘制柜体外沿
		public function drawCabinetBorder(tables:Array,depths:Array,g:Graphics,dx:int,dy:int):void
		{
		...
		//绘制挡水
		public function getWaterHoldingBorder(tables:Array,depths:Array,isView2d:Boolean=true):Array
		{
			var dsWidth:int = isView2d ? 20 : 0;//10;//20;//挡水的宽度
			var dsWidth2:int = isView2d ? 40 : 10;//20;//40;//挡水的宽度
			var cyWidth:int = 30;//台面出沿的宽度
		...
		//绘制挡水
		public function drawWaterHoldingBorder(tables:Array,depths:Array,g:Graphics,dx:int,dy:int):void
		{
			var pss:Array = this.getWaterHoldingBorder(tables,depths);
		...
		//计算台面二维坐标值范围
		public function countBound(tables:Array,depths:Array,max:Point,min:Point):void
		{
			max.x = 0;
			max.y = 0;
			min.x = 100000000;
		...
		static private var _instance:CabinetTableUtils;
		static public function get instance():CabinetTableUtils
		{
			_instance ||= new CabinetTableUtils();
			return _instance;
		}
	}
}
